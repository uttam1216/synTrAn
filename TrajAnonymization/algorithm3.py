...
def privacy_preservation(index, user_dataset, row, threshold):
    print(index)
    ...
    if start_end_node_distance > threshold or lat_distance > threshold or long_distance > threshold:
        return privacy_preservation(index, user_dataset, row, threshold)
...
def main():
    ...
    origianl_dataset = pd.read_csv('data/small_osmnx.csv')
    user_dataset = pd.read_csv('data/small_osmnx.csv')
    # origianl_dataset = origianl_dataset[:20]
    upLat = []
    upLon = []
    upUser = []
    for index, row in origianl_dataset.iterrows():
        updated_latitude, updated_longitude = privacy_preservation(index, user_dataset, row, threshold=1)
    ...
    final_df.to_csv('Data/resultcsv_osmnxV3.csv', index=False)

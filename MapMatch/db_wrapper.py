...
def get_closest_point(lat, lon, segment):
    ...
    linestr = "'LINESTRING({0} {1},{2} {3})'".format(segment['points'][0][0], segment['points'][0][1],
                                                     segment['points'][1][0], segment['points'][1][1])
...
def query_ways_within_radius(g, lat, lon, radius):
    ...
    qstring = "SELECT ST_AsText({1}), osm_id, ST_AsText(ST_Transform(way, {3})), oneway FROM {0} WHERE ST_DWithin(ST_Transform(way, {3}), {1}, {2});".format(
        LINE_TABLE, point_in_merc_str, radius, 900913)
    cur.execute(qstring)
    rows = cur.fetchall()
...
def routingdistance(segment1, segment2):
    ...
    way1_qstring = "SELECT id FROM ways_vertices_pgr ORDER BY the_geom <-> ST_GeometryFromText({0},4326) LIMIT 1;".format(
        way1str)
    way2_qstring = "SELECT id FROM ways_vertices_pgr ORDER BY the_geom <-> ST_GeometryFromText({0},4326) LIMIT 1;".format(
        way2str)
    ...
    qstring = "SELECT cost FROM pgr_aStar('SELECT gid AS id,source,target,length_m AS cost, reverse_cost, x1, y1, x2, y2 FROM ways', {0}, {1});".format(
        way1_id, way2_id)

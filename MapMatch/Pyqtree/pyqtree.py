...
def line_passes_through(rect, a, b):
    ...
    y2 = -a * max_x - b
    x2 = (-b - max_y) / a
    return (y1 >= min_y and y1 <= max_y) or (x1 >= min_x and x1 <= max_x) or (y2 >= min_y and y2 <= max_y) or (
...
class _QuadTree(object):
    ...
    def _add_diagonal_edges(self, rect, edge, a, b):
        ...
        for node in self.nodes:
            if (node.rect[2] >= rect[0] and node.rect[0] <= rect[2] and node.rect[3] >= rect[1] and node.rect[1] <=
                rect[3]) and line_passes_through(rect, a, b):
                node.edges.add(edge)

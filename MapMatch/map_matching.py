...
# from Pyqtree.pyqtree import Edge
# from Pyqtree.pyqtree import Edge
...
def get_nearest_road_segments(currentlon, currentlat, radius, index, g):
...
def calculate_emission_probability(lon, lat, radius, index, g):
    segments = get_nearest_road_segments(lon, lat, radius, index, g)
    if len(segments) == 0:
        return None
    coordinate = (lon, lat)
    for segment in segments:
        segment['closest-point'] = get_closest_point(segment['points'][0][0], segment['points'][0][1],
                                                     segment['points'][1][0], segment['points'][1][1], lon, lat)
        distance1 = great_circle(reverse_coordinate(segment['points'][0]),
                                 reverse_coordinate(segment['closest-point'])).meters
        distance2 = great_circle(reverse_coordinate(segment['points'][1]),
                                 reverse_coordinate(segment['closest-point'])).meters
...
def find_possible_segments(lon, lat, radius, states, index, g):
    segments = calculate_emission_probability(lon, lat, radius, index, g)
    if segments == None:
        return None, None
    probabilities = []
    temp_segments = []
...
def exportData(Results1, Results2, output_matched_points, output_matched_segments):
    with open(output_matched_points, 'a') as out_file:
        for Result in Results1:
            if len(Result) == 2:
                rowString = str(Result[0][0]) + "," + str(Result[0][1]) + "," + str(Result[1][0]) + "," + str(
                    Result[1][1]) + "\n"
            else:
...
# exportData(matched_points, matched_segments,output_matched_points,output_matched_segments)
...
def main(filename, quadspoint, segment):
    ...
    dataset = filename
    output_matched_points = quadspoint
    output_matched_segments = segment

    # print index.intersect_edges()
    ...
    index = Index(BBOX, None, None, None, None, 220, 40)
    ...
    for datum in data:
        ...
        if start == True:
            ...
            # print radius
            # print states
            segments, em = find_possible_segments(currentlon, currentlat, radius, states, index, g)
            # print segments
            # print em
            if segments == None:
            ...
            print "I am in current student id"
            previousstudentID = currentstudentID
            previousdate = currentdate
            previouslat = currentlat
            previouslon = currentlon
...
# main('urecaTestData.csv', 'result_quadtree_points_new.csv', 'result_quadtree_segments_new.csv')
# for i in range(1500):
#     filename = "Files/" + str(i) + "_gps.csv"
#     quads = "Quadpoints/" + str(i) + "_result_quadtree_points.csv"
#     segments = "Segments/" + str(i) + "_result_quadtree_segments.csv"
#     main(filename, quads, segments)

for i in range(1):
    i=99
    filename = "testgps/" + str(i) + "_test_gps.csv"
    quads = "Test_points/" + str(i) + "_test_result_quadtree_points.csv"
    segments = "test_segments/" + str(i) + "_test_result_quadtree_segments.csv"
    main(filename, quads, segments)

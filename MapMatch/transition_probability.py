...
def routing_distance(g, segment1, segment2):
    ...
    if orig_node == dest_node:
        if segment1['furthest-node'] == segment2['furthest-node']:
            return great_circle(reverse_coordinate(segment1['closest-point']),
                                reverse_coordinate(segment2['closest-point'])).meters
        else:
            return great_circle(reverse_coordinate(segment1['closest-point']),
                                reverse_coordinate(segment1['closest-node-coordinate'])).meters + great_circle(
                reverse_coordinate(segment2['closest-point']),
                reverse_coordinate(segment2['closest-node-coordinate'])).meters
    ...
    if segment1['furthest-node'] == route[1]:
        route_length_m = route_length_m - great_circle(reverse_coordinate(segment1['closest-point']),
                                                       reverse_coordinate(segment1['closest-node-coordinate'])).meters
    else:
        route_length_m = route_length_m + great_circle(reverse_coordinate(segment1['closest-point']),
                                                       reverse_coordinate(segment1['closest-node-coordinate'])).meters
    if segment2['furthest-node'] == route[-2]:
        route_length_m = route_length_m - great_circle(reverse_coordinate(segment2['closest-point']),
                                                       reverse_coordinate(segment2['closest-node-coordinate'])).meters
    else:
        route_length_m = route_length_m + great_circle(reverse_coordinate(segment2['closest-point']),
                                                       reverse_coordinate(segment2['closest-node-coordinate'])).meters
...
def transition_probability(g, segment1, segment2):
    ...
    great_circle_distance = great_circle(reverse_coordinate(segment1['original-coordinate']),
                                         reverse_coordinate(segment2['original-coordinate'])).meters

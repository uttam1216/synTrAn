...
def segments_id(filename_segments, filename_points):
    ...
    try:
        ...
        for i in range(len(df)):
            ...
            wayid_points = way_ids_points(result_points)
            if wayid_segments == 0:
        ...
        print(len(df))
        print(len(df1))
        return finallist_segments,finallist_points
...
for i in range(100):
    print(i)
    filename_segments = 'test_segments/' + str(i) + '_test_result_quadtree_segments.csv'
    filename_points = 'Test_points/' + str(i) + '_test_result_quadtree_points.csv'
    finallist_segments,finallist_points=segments_id(filename_segments, filename_points)
    final_data = {'Points': finallist_points, 'Segments': finallist_segments}
    dataframe = pd.DataFrame(final_data)

    dataframe.to_csv("testDataset/"+str(i)+"_mobility_map.csv")


# dataframe.to_csv("mobility_map.csv")
# print(dataframe)
